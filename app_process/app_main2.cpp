...
class AppRuntime : public AndroidRuntime {
   public:
    ...
    void setClassNameAndArgs(const String8 &className, int argc,
                             char *const *argv) {
    ...
    virtual void onVmCreated(JNIEnv *env) {
        if (isXposedLoaded) xposed::onVmCreated(env);
...
#error "Unknown instruction set"
#endif
    const char *androidRoot = getenv("ANDROID_DATA");// /data目录
    LOG_ALWAYS_FATAL_IF(androidRoot == NULL, "ANDROID_DATA environment variable unset");

    char dalvikCacheDir[PATH_MAX];
    const int numChars =
        snprintf(dalvikCacheDir, PATH_MAX, "%s/dalvik-cache/%s", androidRoot, kInstructionSet);
    LOG_ALWAYS_FATAL_IF((numChars >= PATH_MAX || numChars < 0),
                        "Error constructing dalvik cache : %s",
                        strerror(errno));

    int result = mkdir(dalvikCacheDir, 0711);
    LOG_ALWAYS_FATAL_IF((result < 0 && errno != EEXIST),
                        "Error creating cache dir %s : %s", dalvikCacheDir,
                        strerror(errno));
    ...
    LOG_ALWAYS_FATAL_IF((result < 0),
                        "Error changing dalvik-cache ownership : %s",
                        strerror(errno));

    result = chmod(dalvikCacheDir, 0711);
    LOG_ALWAYS_FATAL_IF((result < 0),
                        "Error changing dalvik-cache permissions : %s",
                        strerror(errno));
}
...
static void runtimeStart(AppRuntime &runtime, const char *classname,
    ...
    *(void **)(&ptr2) = dlsym(
        RTLD_DEFAULT,
        "_ZN7android14AndroidRuntime5startEPKcRKNS_6VectorINS_7String8EEE");
...
int main(int argc, char *const argv[]) {
    // app_process在运行过程中，通过Linux下的pctrl系统调用将自己的名字换成了“zygote”
    ...
    // ignore argv[0]。argv[0] 保存的是程序的名称，并且包含了程序所在的完整路径
    argc--;
    argv++;
    ...
    int i;
    for (i = 0; i < argc; i++) {
        if (argv[i][0] != '-') {//argv[i]是一个字符串，argv[i][0]取字符串第一个字符
            break;
        }
        ...
        runtime.addOption(strdup(argv[i]));//strdup(str)新建一个字符串str，对新字符串的修改不影响原字符串
    }
    ...
    if (!className.isEmpty()) {
        ...
        if (property_get(ABI_LIST_PROPERTY, prop, NULL) == 0) {
            LOG_ALWAYS_FATAL(
                "app_process: Unable to determine ABI list from property %s.", ABI_LIST_PROPERTY);
            return 11;
        }
